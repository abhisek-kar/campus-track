...
import {
  dummyFaculty,
  facultyTableheading,
} from "../../services/tableData/facultyTableData";
...
const AdminFaculties = () => {
  ...
  return (
    <div className="w-full px-2">
      ...
      <div className=" mt-5">
        <Table
          headingData={facultyTableheading}
          rowData={dummyFaculty.slice((pageNo - 1) * 10, pageNo * 10)}
        />
      </div>
      {/* show more option */}
      <div
        className="text-themeBlue font-mono font-semibold flex items-center cursor-pointer"
        onClick={() => setShowAllFacultiesModal(true)}
      >
        <span>show more</span>
        <GoArrowDown />
      </div>
      {/* showing students in a scrollable modal on clicking showmore  */}
      {showAllFacultiesModal ? (
        <AllTableDataModal
          showModal={showAllFacultiesModal}
          onClose={() => setShowAllFacultiesModal(false)}
          headingData={facultyTableheading}
          rowData={dummyFaculty}
        />
      ) : (
        <></>
      )}
      {/* pagination */}
      {dummyFaculty.length > 10 ? (
        <div className="flex justify-end text-themeBlue gap-2">
          ...
          <button
            onClick={() => {
              let totalPages =
                dummyFaculty.length % 10 === 0
                  ? dummyFaculty.length / 10
                  : dummyFaculty.length / 10 + 1;
